...
class RequestThrottleSpec extends AnyFunSuite {
  ...
  test("permits beyond the cap wait, then every request completes") {
    ...
    val throttle = new RequestThrottle(maxConcurrent = 2, minIntervalMs = 0)
  ...
  test("permits are released on success, failure, and a synchronous throw") {
    ...
    val throttle = new RequestThrottle(maxConcurrent = 1, minIntervalMs = 0)
  ...
  test("pacing spaces successive starts by the configured interval") {
    ...
    val throttle = new RequestThrottle(maxConcurrent = 4, minIntervalMs = 100, clock)
  ...
  test("pacing is skipped entirely when the interval is zero") {
    ...
    val clock = new ManualClock()
    val throttle = new RequestThrottle(maxConcurrent = 4, minIntervalMs = 0, clock)
  ...
  test("changing the configured cap replaces the gate") {
    val url = "http://identity-change.local"
    val first = RequestThrottle.forProvider("cfgchange", url, Some("2"), None).getOrElse(fail())
    assert(first.maxConcurrent == 2)

    val second = RequestThrottle.forProvider("cfgchange", url, Some("5"), None).getOrElse(fail())
